...
export default function TableMk({ data }: Props) {
      ...
      <table className="">
        <thead>
          <tr className="border-y border-gray-200 text-gray-500">
            ...
            <th className="font-semibold text-sm w-52 text-center">Kode</th>
            <th className="font-semibold text-sm w-xl text-center">Nama</th>
        ...
        <tbody>
          {currentData.slice(0, viewPerPage)?.map((item, index) => (
            ...
            >
              ...
              <td className="flex justify-center py-1"> <h1 className={`${colors[index % colors.length] } px-2 h-fit w-fit rounded-lg text-sm `}>{item.kode_mk}</h1></td>
              <td> {item.nama_matakuliah}</td>
              <td className="">
                <button
                  onClick={() => handleDelete(item.id_mk)}
